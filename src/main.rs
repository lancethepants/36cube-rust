...
fn main() {
    ...
    let mut board = Array::<Tower, Ix2>::from_elem((6, 6), Tower::new(1, Color::Red, false));
...
fn evaluate_cube(
    board: &mut Array<Tower, Ix2>,
    freetowers: &mut Vec<Tower>,
    board_height: &Vec<Vec<i32>>,
    p: Position,
) -> Position {
    if board[[p.row as usize, p.column as usize]].inuse
        && board[[p.row as usize, p.column as usize]].color == Color::Purple
    {
        board[[p.row as usize, p.column as usize]].inuse = false;
        freetowers.push(board[[p.row as usize, p.column as usize]]);
    ...
    if board[[p.row as usize, p.column as usize]].inuse {
        i = board[[p.row as usize, p.column as usize]].color.int_value() + 1;
    }
    ...
    while i < 6 {
        ...
        if index > -1 && color_is_free(&board, p, Color::from_int(i).unwrap()) {
            ...
            board[[p.row as usize, p.column as usize]] = Tower::new(
                (6 - board_height[p.row as usize][p.column as usize])
                    .try_into()
                    .unwrap(),
                Color::from_int(i).unwrap(),
...
fn print_board(board: &Array<Tower, Ix2>) {
    if board[[3, 1]].color == Color::Yellow && board[[3, 3]].color == Color::Orange {
        for i in 0..6 {
            for j in 0..6 {
                if board[[i, j]].inuse {
                    print!(
                        "{}",
                        board[[i, j]].color.to_string().chars().next().unwrap()
                    );
...
fn color_is_free(board: &Array<Tower, Ix2>, p: Position, color: Color) -> bool {
    for i in 0..6 {
        if board[[p.row as usize, i]].color == color && board[[p.row as usize, i]].inuse == true {
            return false;
        }
    }

    for i in 0..6 {
        if board[[i, p.column as usize]].color == color
            && board[[i, p.column as usize]].inuse == true
        {
            return false;
        }
    }
